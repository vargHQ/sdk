...
import { NoirPortrait } from "vargai/characters";
import { fal } from "vargai/ai";

const girl = Image({
  prompt: {
    text: NoirPortrait.prompt,
    images: NoirPortrait.imageRefs,
  },
...
\`\`\`tsx
import { NoirPortrait, Madi } from "vargai/characters";

// Use pre-built characters with image refs
const girl = Image({
  prompt: { text: NoirPortrait.prompt, images: NoirPortrait.imageRefs },
  model: fal.imageModel("nano-banana-pro/edit"),
});

// Or create your own
const robot = Image({ prompt: "blue robot" });

// Reuse same reference for consistent appearance
<Video prompt={{ text: "waving", images: [girl] }} />
<Video prompt={{ text: "dancing", images: [girl] }} />
\`\`\`
