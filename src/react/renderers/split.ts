  ...
  const layers: Layer[] = slotChildren.map(({ path, isVideo, options }, i) => {
    ...
    if (isVideo) {
      return {
        ...
        height: cellHeight,
        resizeMode,
        // Pass slot options for advanced processing
        slotPosition: options.position,
        slotBgBlur: options.bgBlur,
        slotBgDim: options.bgDim,
        slotBgScale: options.bgScale,
        slotBgColor: options.bgColor,
      };
    }
    return {
